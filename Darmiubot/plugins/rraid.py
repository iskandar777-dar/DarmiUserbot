...
from Darmiubot.config import SUDO_USERS
...
from pyrogram.types import (
    ...
    Message)
from Darmiubot.utilities.mongo import kaalub_info, rkaal, runkaal
...
async def replyramd(client: Client, message: Message):
    ...
    failed = 0
    if not user:
        await Kaal.edit("`Balas Ke Pengguna Atau Sebutkan Untuk Mengaktifkan Replyraid `")
        return
    try:
    ...
    if userz.id == mee.id:
        await Kaal.edit("`Darmi Ubot`")
        return
    if await kaalub_info(userz.id):
        await Kaal.edit("`Siapa? Balas Raid Sudah Diaktifkan pada Pengguna itu:/`")
        return
    if int(userz.id) in SUDO_USERS:
        await Kaal.edit("orang itu bagian dari devs saya.")
        return
    ...
    if not chat_dict:
        ...
        return
    await Kaal.edit("`Mengaktifkan Replyraid....!`")
    await rkaal(userz.id, reason)
    gbanned = f"Balas Raid telah Diaktifkan Pada {userz.first_name}"
...
async def dreplyramd(client: Client, message: Message):
    ...
    failed = 0
    if not user:
        await Kaal.edit("`Balas Ke Pengguna Atau Sebutkan Untuk Menonaktifkan Balasan`")
        return
    try:
        userz = await client.get_users(user)
    except:
    ...
    if not await kaalub_info(userz.id):
        await Kaal.edit("`Ketika saya Replyraid Diaktifkan? Pada Pengguna Itu?:/`")
        return
    ...
    if not chat_dict:
        Kaal.edit("`Anda Tidak Memiliki Obrolan! Sangat sedih`")
        return
    await Kaal.edit("`Menonaktifkan Serangan Balas Dendam....!`")
    await runkaal(userz.id)
    ungbanned = f"**Serbuan Balas Dendam dinonaktifkan [{userz.first_name}](tg://user?id={userz.id})"
